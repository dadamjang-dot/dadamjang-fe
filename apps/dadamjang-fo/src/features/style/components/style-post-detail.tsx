      ...
      ListHeaderComponent={
        <View>
          <FlatList
            ...
            renderItem={({ item }) => (
              <View style={s.page}>
                <Image contentFit="cover" source={item} style={s.image} transition={120} />
              </View>
            )}
          ...
          <View style={s.pageIndicator}>
            {post.imageUrls.map((imageUrl, index) => <View key={`${imageUrl}-dot-${index}`} style={s.dot} />)}
          </View>
          <View style={s.content}>
            <View style={s.authorRow}>
              ...
              <Button
                ...
                accessibilityState={{ selected: post.isLiked, disabled: isLikePending }}
            ...
            {post.brandTags.length ? (
              <View style={s.tagRow}>
                {post.brandTags.map((tag) => <Text key={tag.brandId} style={s.brandTag}>@{tag.name}</Text>)}
              </View>
            ) : null}
...
const s = StyleSheet.create({
  ...
  pageIndicator: { flexDirection: "row", justifyContent: "center", gap: 4, paddingVertical: 10 },
  dot: { width: 5, height: 5, borderRadius: 3, backgroundColor: colors.line },
  content: { gap: spacing.md, padding: 16 },
  authorRow: { flexDirection: "row", alignItems: "center", justifyContent: "space-between", gap: spacing.md },
  authorBlock: { gap: 4 },
  ...
  sectionTitle: { paddingTop: spacing.sm, color: colors.ink, fontSize: 16, fontWeight: "700" },
  listContent: { paddingBottom: 24 },
  productButton: { flexDirection: "row", alignItems: "center", gap: 12, minHeight: 76, paddingHorizontal: 16, borderTopWidth: 1, borderTopColor: colors.line },
  productImage: { width: 56, height: 56, borderRadius: 8, backgroundColor: colors.primarySoft },
  productPlaceholder: { width: 56, height: 56, borderRadius: 8, backgroundColor: colors.primarySoft },
