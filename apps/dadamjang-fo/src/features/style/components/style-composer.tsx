...
const StyleComposer = ({ onClose }: StyleComposerProps) => {
  ...
  const toggleProduct = (product: PurchasedStyleProduct) => {
    ...
    setBrandTagIds((current) =>
      current.filter((brandId) =>
        next.some((selected) => selected.brandId === brandId),
      ),
    );
  };
  ...
  const insertBrandMention = (product: PurchasedStyleProduct) => {
    ...
    if (product.brandId)
      setBrandTagIds((current) =>
        current.includes(product.brandId!)
          ? current
          : [...current, product.brandId!],
      );
  };
    ...
    try {
      ...
      setMessage(
        error instanceof Error
          ? error.message
          : "스타일을 등록하지 못했어요. 입력한 내용을 확인해 주세요.",
      );
    } finally {
      submitLock.current = false;
    ...
    <View style={s.container}>
        ...
        <View style={s.chipRow}>
          {categories.map(({ key, label }) => (
            <Button
              ...
              style={[s.chip, category === key && s.selectedChip]}
              variant="secondary"
            />
          ))}
        </View>
        ...
        {selectedProducts.length ? (
          <View style={s.selectedProductList}>
            {selectedProducts.map((product) => (
              <Text key={product.productId} style={s.selectedProduct}>
      ...
      >
        <View style={s.modalContainer}>
          ...
          {purchasedProducts.isLoading ? (
            <Text style={s.helper}>구매 내역을 불러오는 중이에요.</Text>
          ) : null}
          {purchasedProducts.isError ? (
            <Text style={s.error}>구매 내역을 불러오지 못했어요.</Text>
          ) : null}
          <LegendList
            accessibilityLabel="구매한 상품 목록"
