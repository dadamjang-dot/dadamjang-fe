        ...
        <VStack alignment="leading" spacing={12} modifiers={[padding({ horizontal: 16, vertical: 8 })]}>
          {cart.items.map((item) => (
            <VStack
              ...
              alignment="leading"
              spacing={8}
              modifiers={[background(colors.surface), cornerRadius(18), padding({ all: 12 })]}>
              <RNHostView matchContents>
                <Image source={item.product.imageUrls[0]} style={styles.image} contentFit="cover" />
              </RNHostView>
              <Text modifiers={[font({ size: 17, weight: 'semibold' })]}>{item.product.title}</Text>
              <Text modifiers={[font({ size: 14 }), foregroundStyle(colors.muted)]}>
                {item.sku.optionName} / {item.quantity}개
              </Text>
              <Button label="수량 +" onPress={() => onChangeQuantity(item.sku.skuId, item.quantity + 1)} />
              <Button
                label="수량 -"
          ...
          <Button label="주문하기" onPress={onCheckout} />
          <Button label="mock 결제 실패 테스트" role="destructive" onPress={onCheckoutFailure} />
        </VStack>
      </Host>
    </ScrollView>
  );
