...
export const validatePassword = (password: string) => {
  ...
  if (utf8ByteLength(password) > 72)
    return "비밀번호를 72바이트 이하로 입력해 주세요.";
  return null;
};
...
export const getConsentSelectionState = (
  ...
  const selectedCount = documents.filter(({ documentId }) =>
...
export const hasRequiredConsents = (
  ...
  documents.every(
    ({ documentId, required }) =>
      !required || selectedDocumentIds.has(documentId),
  );
...
export const resolveAuthReturnTo = (returnTo: string | undefined) => {
  ...
  return allowedPathPrefixes.some((prefix) => returnTo.startsWith(prefix))
    ? returnTo
    : "/";
};
...
export const toConsentAcceptances = (
  ...
  documents.map(({ documentId }) => ({
    documentId,
    agreed: selectedDocumentIds.has(documentId),
  }));
