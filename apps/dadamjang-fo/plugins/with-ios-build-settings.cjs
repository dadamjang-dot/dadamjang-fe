...
const createPodDeploymentTargetPatch = (deploymentTarget) =>
  [
...
const removeDuplicatedLibcxxFlag = (value) =>
  Array.isArray(value)
    ? value.filter((flag) => flag !== DUPLICATED_LIBCXX_FLAG)
    : value;

const withPodDeploymentTarget = (config, { deploymentTarget }) =>
...
const withXcodeBuildSettingsFallback = (config) =>
...
const withIosBuildSettings = (config, options = {}) => {
