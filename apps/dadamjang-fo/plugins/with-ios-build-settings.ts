...
const removeDuplicatedLibcxxFlag = (value: unknown) =>
  Array.isArray(value)
    ? value.filter((flag) => flag !== DUPLICATED_LIBCXX_FLAG)
    : value;

const withPodDeploymentTarget: ConfigPlugin<
  Required<IosBuildSettingsPluginOptions>
...
const withXcodeBuildSettings: ConfigPlugin = (config) =>
  withXcodeProject(config, (config) => {
    const shellScriptBuildPhases = (config.modResults.hash.project.objects
      .PBXShellScriptBuildPhase ?? {}) as XcodeObjects;
    const buildConfigurations = (config.modResults.hash.project.objects
      .XCBuildConfiguration ?? {}) as XcodeObjects;
    ...
    for (const buildPhase of Object.values(shellScriptBuildPhases)) {
      ...
      if (!buildPhase.shellScript.includes(SENTRY_DEBUG_SYMBOLS_SCRIPT))
        continue;
...
const withIosBuildSettings: ConfigPlugin<IosBuildSettingsPluginOptions> = (
  ...
  const deploymentTarget =
    options.deploymentTarget ?? DEFAULT_DEPLOYMENT_TARGET;
